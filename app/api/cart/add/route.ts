...
export async function POST(req: Request) {
    try {
        ...
        if (Object.keys(reqBody).length < 5 || !reqBody.productId) {
        ...
        const newCartItem = {
            ...
            color: productItem.characteristics?.color ?? null,
            category: productItem.category,
        };

        const { insertedId } = await db.collection('cart').insertOne(newCartItem); // Виправлено синтаксис
