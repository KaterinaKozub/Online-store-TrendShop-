...
import { IAddProductsFromLSToCartFx, ICartItem } from "@/types/cart"
import { handleJWTError } from "./errors"
...
export const getNewAndBestsellerGoods = async (db: Db, fieldName: string) => {
  ...
  // Збираємо всі товари, крім взуття
  const allItems = [
    ...clothes
      .filter(
        (item) =>
  ...
  // Перемішуємо всі товари і обмежуємо результат до 4 елементів
  return shuffle(allItems).slice(0, 4);
};
