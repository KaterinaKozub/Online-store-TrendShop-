...
export const addCartItemToLS = (
  ...
  const existingItem = cartFromLS.find(
    (item) =>
      item.productId === product._id && (item.size === selectedSize || !item.size)
  )
  ...
  // Якщо товару в кошику немає, додаємо новий
  const cart = [
    ...cartFromLS,
    {
      clientId,
...
export const addProductToCartBySizeTable = (
  ...
  if (productsWithoutSizes.includes(product.type)) {
    // Для товарів без розміру додаємо кілька разів без перевірки розміру
